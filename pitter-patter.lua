        ...
        elseif d == 1 or d == -1 then
            if params:get("main_play") == 1 then
                ...
                sequencers[params:get("main_sequence")]:update(
                    divisions[sequencers[params:get("main_sequence")]:get_param("division")])
            end
        end
    elseif k == 3 then
        if is_shift then
            ...
            -- change note offset
            sequencers[params:get("main_sequence")].note_offset =
                sequencers[params:get("main_sequence")].note_offset + d
        end
    end
    debounce_show_grid = debounce_show_grid_time
end
...
function key(k, z)
    ...
    elseif z == 1 and k == 2 then
        if not is_shift then
            ...
            sequencers[params:get("main_sequence")]:set_param("mute", sequencers[params:get("main_sequence")]:get_param(
                "mute") == 0 and 1 or 0)
        else
...
function redraw()
    ...
    for i, v in ipairs(visual) do
        for j, u in ipairs(v) do
            ...
            screen.stroke()
            if u > 0 then
                screen.rect((j * grid_square_size) + grid_x + 1,
                    i * grid_square_size - grid_square_size / 2 + 2 + grid_y, grid_square_size - 3, grid_square_size - 3)
    ...
    screen.move(0, 64 - 2)
    screen.text(
        params:string("main_play") .. sequencers[params:get("main_sequence")]:get_param_str("direction") .. " " ..
    ...
    local note_string =
        musicutil.note_num_to_name(sequencers[params:get("main_sequence")]:get_note_from_index(1), true) .. " to " ..
            musicutil.note_num_to_name(sequencers[params:get("main_sequence")]:get_note_from_index(grid_.width - 1),
                true)
    ...
    for i, v in ipairs(velocity_profile) do
        ...
        screen.circle(128 - (#velocity_profile + 0.5) * 1.5 * velocity_spacing + i * velocity_spacing * 1.5 + 1,
            64 - 11, velocity_spacing / 2)
        screen.fill()
    end
    ...
    for j, dev in pairs(midi.devices) do
        if dev.port ~= nil then
            ...
            conn.event = function(data)
                ...
                if d.ch ~= nil and d.ch ~= midi_channels[params:get("main_midi_channel")] and
                    params:get("main_midi_channel") > 2 then
                    do
                        return
                    end
                end
