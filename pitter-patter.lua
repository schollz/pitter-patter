  ...
  elseif z == 1 and k == 2 then
    if not is_shift then
      ...
      sequencers[params:get("main_sequence")]:set_param("mute",
                                                        sequencers[params:get("main_sequence")]:get_param("mute") == 0 and
                                                            1 or 0)
    else
...
function redraw()
  ...
  -- show length of seequence
  screen.move(128, 64 - 8)
  screen.text_right(sequencers[params:get("main_sequence")]:get_param("limit") .. " steps")
  ...
  local params_menu = {
    {
      ...
      action=function(v)
        if v == 0 then for i = 1, 4 do sequencers[i]:reset_timer() end end
      end
    }, {
      id="midi_input",
